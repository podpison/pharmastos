...
export const Contacts: React.FC<Props> = ({ className, phoneNumbers, geopoint, email }) => {
  let Phones = phoneNumbers.map((p, index) => <a key={index} className='text contacts__phone' href={`tel: ${p}`}><span>{p}</span></a>)

  return <div className={classNameHelper('contacts', {}, className)}>
    <div className="contacts__phones">
      {Phones}
    </div>
    {geopoint && <a target="_blank" rel="noopener noreferrer" href={geopoint.link} className="text contacts__geopoint">{geopoint.name}</a>}
